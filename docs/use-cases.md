...
- アクター: 認証済みユーザー (許可ドメイン制限を通過、Clerk Org 必須 ON)
- 前提条件:
...
sequenceDiagram
    ...
    API->>API: locals.auth() → orgId<br/>assignmentApiUpdateSchema.safeParse(body)
    API->>DDB: updateAssignment(orgId, prevStartDate, payload)
...
- **未認証 / Org 未指定**: API が 401 / 403 → 画面 revert + toast「保存に失敗しました」
...
- DB アクセス: [`web/src/lib/repository/assignment.ts`](../web/src/lib/repository/assignment.ts) の `deleteAssignment(orgId, startDate, id)`
...
sequenceDiagram
    ...
    alt キャンセル
        ...
        Server->>DDB: DeleteItem<br/>pk=ORG#{orgId}, sk=ASN#{startDate}#{id}
...
sequenceDiagram
    ...
    alt キャンセル
        ...
        SK->>Server: POST ?/deleteResource<br/>FormData(id)
        Server->>Repo: deleteResource(orgId, id)
        Repo->>DDB: Query<br/>pk=ORG#X AND begins_with(sk, "ASN#")<br/>FilterExpression: resourceId == id
