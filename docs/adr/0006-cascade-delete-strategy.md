...
**`web/src/lib/repository/resource.ts` の `deleteResource(orgId, id)`** (Project 側も同様):

1. `Query(pk = ORG#X AND begins_with(sk, "ASN#"))` + `FilterExpression: resourceId == id` で関連 Assignment の SK 一覧を取得 (pagination 対応)
2. `TransactWriteItems` で:
   - Resource 自身の Delete (`pk=ORG#X, sk=RES#id`)
   - 各関連 Assignment の Delete (`pk=ORG#X, sk=ASN#date#id`)
